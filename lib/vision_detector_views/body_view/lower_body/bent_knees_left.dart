...
class Detector_bent_knees_left implements Detector_default{
  ...
  void poseDetector() {
    // double z1 = posedata[78]!;
    // double x2 = posedata[40]!;
    //
    // // 計算距離
    // double dist = zz(z1);
    //
    ...
    if (this.startdDetector) {
      ...
      if (zz(posedata[74]!, posedata[66]!)<0 && zz(posedata[74]!, posedata[66]!)<0 //手跟肩膀的深度
      ) {
  ...
  Future<void> posesounder(bool BOO) async {
    ...
    if(BOO){
      ...
      await _audioPlayer.play(AssetSource('$baseAudioPath/lower/bent_knees_left.${getAudioDataForm()}'));
    }
  }
